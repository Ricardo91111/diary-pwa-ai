...
app.post("/api/weekly-summary", async (req, res) => {
  ...
  if (
    ...
    return res.status(400).json({ ok: false, error: "invalid_stats" });
  }
  ...
  for (const d of days) {
    if (!d || typeof d !== "object" || !isDateKey(d.date) || !Array.isArray(d.tasks)) {
      return res.status(400).json({ ok: false, error: "invalid_days_item" });
    }
    if (typeof d.weekdayLabel !== "string") {
      return res.status(400).json({ ok: false, error: "invalid_weekdayLabel" });
    }
    if (d.tasks.length > 200) {
      return res.status(400).json({ ok: false, error: "too_many_tasks" });
    }
  ...
  if (!expectedSet.has(range.start) || !expectedSet.has(range.end)) {
    return res.status(400).json({ ok: false, error: "range_not_7_days" });
  }
  ...
  if (!AI_BASE_URL || !AI_API_KEY || !AI_MODEL) {
    return res.status(500).json({ ok: false, error: "missing_ai_env" });
  }
